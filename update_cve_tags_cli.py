...
def update_attribute_sighting(data, misp):
    for cve, ips in data.items():
        for ip in ips:
            doc = {}
            doc["values"] = [cve]
            doc["source"] = ip
            ...
            print ("Input JSON to MISP: {}".format(doc))
            res = misp.set_sightings(doc)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--file", required=False, default='vuln_data.csv', help="CSV file with information about vulnerabilities.")
    ...
    parser.add_argument("-ssl", "--verify_cert", required=False, default=True, help="Verify MISP certificate?")
